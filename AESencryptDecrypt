...
public class AESencryptDecrypt {
	...
	public static char[] encryptFullArray(char[] in, char[] key){
		char[] out = new char[((in.length-1)/16)*16+16];
		...
		for(int i=0;i<out.length/16;i++){
			...
			temp=aesEncryptBlock(temp,expand);
			for(int j=0;j<16;j++){
				out[i*16+j]=temp[j];
			}
		}
	...
	public static char[] decryptFullArray(char[] in, char[] key){
		char[] out = new char[((in.length-1)/16)*16+16];
		...
		for(int i=0;i<out.length/16;i++){
			...
			temp=aesDecryptBlock(temp,expand);
			for(int j=0;j<16;j++){
				out[i*16+j]=temp[j];
			}
		}
	...
	private static char[] subBytes(char[] in){
		for(int i=0; i<in.length;i++){
			in[i] = sbox[((int)in[i]) & 0xFF];
		}//*/
		return in;
	}
	
	private static char[] subBytesInv(char[] in){
		for(int i=0; i<in.length;i++){
			in[i] = sboxInv[((int)in[i]) & 0xFF];
		}//*/
		return in;
	}
	
	private static char[] addRoundKey(char[] in, char[] roundKey){
		for(int i=0;i<in.length;i++){
	...
	private static char galoisMult(char a, char b){
		char r=0;
		char hiBitSet=0;
		for(int i=0;i<8;i++){
			if ((b & 1) == 1){
	...
	private static char[] createRoundKey(char[] expandKey, int n){
	...
	private static char[] aesEncryptBlock(char[] in, char[] expandKey){
	...
	private static char[] aesDecryptBlock(char[] in, char[] expandKey){
